...
def seed_demo():
    ...
    if read("cursos").empty:
        ...
        ]).to_csv(CSVS["cursos"], index=False)
    ...
    if read("professores").empty:
        pd.DataFrame([
            {"id": 1, "nome": "Carlos Souza",   "email": "carlos@escola.com",  "dias_disponiveis": "Segunda,Terca,Quarta,Quinta,Sexta", "max_aulas_dia": 4, "id_turma": ""},
            {"id": 2, "nome": "Ana Lima",        "email": "ana@escola.com",     "dias_disponiveis": "Segunda,Terca,Quinta,Sexta",        "max_aulas_dia": 3, "id_turma": ""},
            {"id": 3, "nome": "Roberto Nunes",   "email": "roberto@escola.com", "dias_disponiveis": "Segunda,Quarta,Sexta",              "max_aulas_dia": 4, "id_turma": ""},
        ]).to_csv(CSVS["professores"], index=False)

    # Disciplinas
    if read("disciplinas").empty:
        pd.DataFrame([
            {"id": 1, "nome": "Matematica",  "carga_horaria": 4, "id_professor": 1, "duracao_blocos": 1},
            {"id": 2, "nome": "Portugues",   "carga_horaria": 4, "id_professor": 2, "duracao_blocos": 1},
            {"id": 3, "nome": "Fisica",      "carga_horaria": 3, "id_professor": 3, "duracao_blocos": 1},
            {"id": 4, "nome": "Historia",    "carga_horaria": 2, "id_professor": 2, "duracao_blocos": 1},
            {"id": 5, "nome": "Database Design", "carga_horaria": 2, "id_professor": 2, "duracao_blocos": 2},
        ]).to_csv(CSVS["disciplinas"], index=False)

    # Turmas
    if read("turmas").empty:
        pd.DataFrame([
            {"id": 1, "nome": "Turma A", "quantidade_alunos": 35, "id_curso": 1},
            {"id": 2, "nome": "Turma B", "quantidade_alunos": 32, "id_curso": 2},
            {"id": 3, "nome": "Turma C", "quantidade_alunos": 28, "id_curso": 3},
            {"id": 4, "nome": "Turma D", "quantidade_alunos": 30, "id_curso": 4},
